...
def create_actionrow(*components: dict) -> dict:
    ...
    if not components or len(components) > 5:
        raise IncorrectFormat(
            "Number of components in one row should be between 1 and 5."
        )
    if (
        ...
        raise IncorrectFormat(
            "Action row must have only one select component and nothing else"
        )
...
def get_messages_ids(
    message: typing.Union[int, discord.Message, list]
) -> typing.Iterator[int]:
    """
