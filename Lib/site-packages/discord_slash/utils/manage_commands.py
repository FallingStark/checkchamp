    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            if resp.status == 429:
                _json = await resp.json()
                await asyncio.sleep(_json["retry_after"])
                return await get_all_guild_commands_permissions(
                    bot_id, bot_token, guild_id
                )
            if not 200 <= resp.status < 300:
                raise RequestFailure(resp.status, await resp.text())
            return await resp.json()
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            if resp.status == 429:
                _json = await resp.json()
                await asyncio.sleep(_json["retry_after"])
                return await get_guild_command_permissions(
                    bot_id, bot_token, guild_id, command_id
                )
            if not 200 <= resp.status < 300:
                raise RequestFailure(resp.status, await resp.text())
            return await resp.json()
...
async def update_single_command_permissions(
...
async def update_guild_commands_permissions(
...
def create_multi_ids_permission(
    ids: typing.List[int],
    id_type: typing.Union[int, SlashCommandPermissionType],
    permission: bool,
):
    """
    ...
    if allowed_roles:
        permissions.extend(
            create_multi_ids_permission(
                allowed_roles, SlashCommandPermissionType.ROLE, True
            )
        )
    if allowed_users:
        permissions.extend(
            create_multi_ids_permission(
                allowed_users, SlashCommandPermissionType.USER, True
            )
        )
    if disallowed_roles:
        permissions.extend(
            create_multi_ids_permission(
                disallowed_roles, SlashCommandPermissionType.ROLE, False
            )
        )
    if disallowed_users:
        permissions.extend(
            create_multi_ids_permission(
                disallowed_users, SlashCommandPermissionType.USER, False
            )
        )
