...
class InteractionContext:
    ...
    def __init__(
        ...
        self._deferred_hidden = (
            False  # To check if the patch to the deferred response matches
        )
    ...
    def deffered(self):
        warn(
            "`deffered` as been renamed to `deferred`.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class ComponentContext(InteractionContext):
    ...
    def __init__(
        ...
        self.origin_message_id = (
            int(_json["message"]["id"]) if "message" in _json.keys() else None
        )
    ...
    async def defer(
        self, hidden: bool = False, edit_origin: bool = False, ignore: bool = False
    ):
        """
        'Defers' the response, showing a loading state to the user
...
class MenuContext(InteractionContext):
    ...
    def __init__(
        ...
        super().__init__(_http=_http, _json=_json, _discord=_discord, logger=logger)
        self.name = self.command = self.invoked_with = _json["data"][
            "name"
        ]  # This exists.
        self.context_type = _json["type"]
        self._resolved = (
            self.data["resolved"] if "resolved" in self.data.keys() else None
        )
        self.target_message = None
        self.target_author = None
        self.target_id = (
            self.data["target_id"] if "target_id" in self.data.keys() else None
        )
