...
def _get_val(
    d: dict, key
):  # util function to get value from dict with fallback to None key
    try:
        value = d[key]
...
class SlashCommand:
    ...
    async def sync_all_commands(
        ...
        for scope in permissions_map:
            ...
            if len(existing_perms) != len(new_perms):
                ...
                for existing_perm in existing_perms:
                    existing_perms_model[
                        existing_perm["id"]
                    ] = model.GuildPermissionsData(**existing_perm)
                for new_perm in new_perms:
                    if new_perm["id"] not in existing_perms_model:
                        changed = True
                        break
                    if existing_perms_model[
                        new_perm["id"]
                    ] != model.GuildPermissionsData(**new_perm):
                        changed = True
                        break

            if changed:
                self.logger.debug(
                    f"Detected permissions changes on {scope}, updating them"
                )
                await self.req.update_guild_commands_permissions(scope, new_perms)
            else:
                self.logger.debug(
                    f"Detected no permissions changes on {scope}, skipping"
                )
        ...
        if delete_perms_from_unused_guilds:
            ...
            other_guilds = [
                guild.id
                for guild in self._discord.guilds
                if guild.id not in permissions_map.keys()
            ]
            for guild in other_guilds:
                with suppress(discord.Forbidden):
                    self.logger.debug(f"Deleting permissions from {guild}")
                    existing_perms = await self.req.get_all_guild_commands_permissions(
                        guild
                    )
    ...
    def add_component_callback(
        ...
        *,
        ...
        message_ids = (
            list(get_messages_ids(messages)) if messages is not None else [None]
        )
        custom_ids = (
            list(get_components_ids(components)) if components is not None else [None]
        )
        ...
        if message_ids == [None] and custom_ids == [None]:
            raise error.IncorrectFormat(
                "You must specify messages or components (or both)"
            )
    ...
    def _add_comp_callback_obj(self, callback_obj):
        ...
        for message_id, custom_id in callback_obj.keys:
            self._register_comp_callback_obj(
                callback_obj, message_id, custom_id, component_type
            )

    def _register_comp_callback_obj(
        self, callback_obj, message_id, custom_id, component_type
    ):
    ...
    def remove_component_callback(
        ...
        try:
            ...
            if not self.components[message_id][
                custom_id
            ]:  # delete dict nesting levels if empty
    ...
    def remove_component_callback_obj(
        self, callback_obj: model.ComponentCallbackObject
    ):
        """
        ...
        if hasattr(func, "on_error"):
            if func.on_error is not None:
                try:
                    ...
                    self.logger.error(
                        f"{ctx.command}:: Error using error decorator: {e}"
                    )
        return False
    ...
    async def _on_context_menu(self, to_use):
        ...
        if to_use["data"]["name"] in self.commands["context"]:
            ...
            if (
                cmd_name not in self.commands["context"]
                and cmd_name in self.subcommands
            ):
